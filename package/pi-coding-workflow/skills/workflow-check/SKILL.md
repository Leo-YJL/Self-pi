...
- Maintain `implement.jsonl` / `check.jsonl` with `workflow_run` actions (`init_manifests`, `upsert_manifest_entry`, `remove_manifest_entry`, `sync_manifest_from_diff`) instead of hand-writing JSONL. For `sync_manifest_from_diff`, use dry-run candidates plus explicit entries/reasons before execute.
